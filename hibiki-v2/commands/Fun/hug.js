...
class hugCommand extends Command {
  ...
  run(msg, args) {
    ...
    if (!args[0] || member.id == msg.author.id) return msg.channel.createMessage(this.bot.embed("❌ Error", "Hug someone other than yourself for once!", "error"));

    // Sends the embed
    weebSH.images.getRandomImage("hug")
      .then(image => msg.channel.createMessage({
        embed: {
          description: `🤗 **${user}** was hugged by **${msg.author.username}!**`,
          image: {
            url: image.url,
          },
