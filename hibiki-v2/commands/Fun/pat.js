...
class patCommand extends Command {
  constructor(...args) {
    super(...args, {
      aliases: ["headpat", "pet"],
      description: "Gives another user a headpat.",
      usage: "<@user>",
      cooldown: 3000,
    });
  }
  ...
  run(msg, args) {
    ...
    if (!args[0] || member.id == msg.author.id) return msg.channel.createMessage(this.bot.embed("❌ Error", "Pat someone other than yourself, incel.", "error"));

    // Sends the embed
    weebSH.images.getRandomImage("pat")
      .then(image => msg.channel.createMessage({
        embed: {
          description: `💓 **${user}** was patted by **${msg.author.username}!**`,
          image: {
            url: image.url,
          },
