...
class punchCommand extends Command {
  constructor(...args) {
    super(...args, {
      aliases: ["beatup"],
      description: "Punches another user.",
      usage: "<@user>",
      cooldown: 3000,
    });
  }
  ...
  run(msg, args) {
    ...
    if (!args[0] || member.id == msg.author.id) return msg.channel.createMessage(this.bot.embed("❌ Error", "Stop hitting yourself!", "error"));

    // Sends the embed
    weebSH.images.getRandomImage("punch")
      .then(image => msg.channel.createMessage({
        embed: {
          description: `💢 **${user}** was punched by **${msg.author.username}**`,
          image: {
            url: image.url,
          },
