...
class holdhandsCommand extends Command {
  constructor(...args) {
    super(...args, {
      aliases: ["handhold"],
      description: "Holds hands with another user.",
      usage: "<@user>",
      cooldown: 3000,
    });
  }
  ...
  run(msg, args) {
    ...
    if (!args[0] || member.id == msg.author.id) return msg.channel.createMessage(this.bot.embed("❌ Error", "Aww, imagining holding someone else's hand? You're so lonely...", "error"));

    // Sends the embed
    weebSH.images.getRandomImage("handholding")
      .then(image => msg.channel.createMessage({
        embed: {
          description: `**💚 ${msg.author.username}** is holding **${user}**'s hands 👀`,
          image: {
            url: image.url,
          },
