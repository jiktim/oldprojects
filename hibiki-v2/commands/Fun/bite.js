...
class biteCommand extends Command {
  ...
  run(msg, args) {
    ...
    // Handler for if the author mentions themselves
    if (!args[0] || member.id == msg.author.id) return msg.channel.createMessage(this.bot.embed("❌ Error", "Stop biting yourself, that isn't healthy.", "error"));

    // Sends the embed
    weebSH.images.getRandomImage("bite")
      .then(image => msg.channel.createMessage({
        embed: {
          description: `**❤ ${user}** was bitten by **${msg.author.username}!**`,
          image: {
            url: image.url,
          },
