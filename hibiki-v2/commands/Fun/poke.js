...
class pokeCommand extends Command {
  constructor(...args) {
    super(...args, {
      description: "Pokes another user.",
      usage: "<@user>",
      cooldown: 3000,
    });
  }
  ...
  run(msg, args) {
    ...
    if (!args[0] || member.id == msg.author.id) return msg.channel.createMessage(this.bot.embed("❌ Error", "Stop poking yourself!", "error"));

    // Sends the embed
    weebSH.images.getRandomImage("poke")
      .then(image => msg.channel.createMessage({
        embed: {
          description: `👉 **${user}** was poked by **${msg.author.username}!**`,
          image: {
            url: image.url,
          },
