...
class tickleCommand extends Command {
  constructor(...args) {
    super(...args, {
      description: "Tickles another user.",
      usage: "<@user>",
      cooldown: 3000,
    });
  }
  ...
  run(msg, args) {
    ...
    if (!args[0] || member.id == msg.author.id) return msg.channel.createMessage(this.bot.embed("❌ Error", "Tickle someone else, freak!", "error"));

    // Sends the embed
    weebSH.images.getRandomImage("tickle")
      .then(image => msg.channel.createMessage({
        embed: {
          description: `💗 **${user}** is being tickled by **${msg.author.username}!**`,
          image: {
            url: image.url,
          },
