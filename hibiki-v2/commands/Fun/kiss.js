...
class kissCommand extends Command {
  ...
  run(msg, args) {
    ...
    if (!args[0] || member.id == msg.author.id) return msg.channel.createMessage(this.bot.embed("❌ Error", "Kiss someone other than yourself, weirdo.", "error"));

    // Sends the embed
    weebSH.images.getRandomImage("kiss")
      .then(image => msg.channel.createMessage({
        embed: {
          description: `💙 **${user}** was kissed by **${msg.author.username}!**`,
          image: {
            url: image.url,
          },
